...
// v3.104.0 -- 2 mekanisme independen:
// 1) kategori (driver.notifKategori, diatur di Data Sopir, dikunci PIN) --
//    HANYA berlaku utk doc/service/etoll/bbm. Kategori 'booking' SENGAJA
//    DIKECUALIKAN -- tidak lagi diatur lewat Data Sopir sama sekali.
// 2) targetDriverIds (diisi dari checklist "Kirim Reminder Telegram Ke" di
//    form Booking, TANPA PIN) -- HANYA berlaku utk alert booking-h1.
...
const NOTIF_SETTINGS_DEFAULT = { docExpiryDays: 30, serviceWarnPct: 80, bbmMinDefault: 20, etollMinDefault: 25000, checkIntervalMinutes: 60 };
...
function computeAlerts() {
  ...
  (state.bookings || []).forEach(b => {
    ...
    alerts.push({ id: 'booking-h1-' + b.id, ic: '📅', level: 'warn', judul: 'Booking besok', keterangan: `${carLabel(b.carId)} — ${b.tujuan || '(tanpa tujuan)'}${b.jamMulai ? ' · ' + b.jamMulai : ''}`, targetDriverIds: Array.isArray(b.notifDriverIds) ? b.notifDriverIds : null });
  });
...
// app (menu Pengaturan, dikunci PIN). Cron tetap tick tiap 5 menit (lihat
// telegram-notify.yml) -- kalau belum waktunya SESUAI interval yang
// diatur, lewati bagian pengecekan alert berkala ini SAJA. TIDAK
// memengaruhi 🧾 Resi Perjalanan di atas, yang sudah diproses sebelum baris
// ini dan sengaja selalu jalan tiap tick.
...
const dueForAlertCheck = (Date.now() - lastAlertCheckAt) >= checkIntervalMs;

const LEVEL_RANK = { warn: 1, danger: 2 };
const toSendCandidates = dueForAlertCheck ? alerts.filter(a => {
...
// dedup BARU: kalau BELUM WAKTUNYA cek (throttle checkIntervalMinutes),
// dedup dibiarkan PERSIS seperti sebelumnya -- supaya begitu waktunya tiba,
// perubahan yang terjadi SELAMA masa throttle tetap terdeteksi sebagai
// "baru" dan benar terkirim (bukan dianggap sudah, padahal belum pernah
// benar-benar dikirim). Alert yang DISUPRES jam sunyi juga sama alasannya.
const newDedup = {};
alerts.forEach(a => {
  if (!dueForAlertCheck || suppressedIds.has(a.id)) {
...
if (toSend.length === 0) {
  ...
  if (!dueForAlertCheck) {
    ...
    console.log(`⏳ Belum waktunya cek notifikasi berkala lagi (interval diatur: tiap ${ns.checkIntervalMinutes} menit, ~${Math.max(menitLagi,0)} menit lagi). Resi Perjalanan tetap sudah diproses di atas kalau ada.`);
  } else {
